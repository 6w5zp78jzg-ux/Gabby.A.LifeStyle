"use client";
import { useState } from "react"; // Importamos useState para el desplegable
...
export default function HeroMagazine() {
  const [showSocials, setShowSocials] = useState(false); // Estado para el desplegable
  ...
  return (
    <section className="relative h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-black text-white">
      ...
      <div className="relative z-20 flex flex-col items-center text-center px-4 w-full mt-12 md:mt-20">
        ...
        {/* Separador */}
        <div className="mt-20 md:mt-32 flex items-center justify-center w-full max-w-sm gap-4">
        ...
        <p className={`${montserrat.className} uppercase tracking-[0.3em] text-[8px] md:text-[10px] mt-6 md:mt-8 opacity-90`}>
          Lifestyle. Purpose. Elevated.
        </p>

        {/* BOTÓN: ENTER MY WORLD (Corregido para Scroll) */}
        ...
        <div className="mt-8 md:mt-12 relative flex flex-col items-center">
          <button 
          ...
          >
            ...
            <motion.span animate={{ rotate: showSocials ? 180 : 0 }}>
          ...
          {/* Iconos que aparecen al hacer clic */}
          <AnimatePresence>
            {showSocials && (
              <motion.div 
                initial={{ opacity: 0, y: -10 }}
