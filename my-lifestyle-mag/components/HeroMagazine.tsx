...
export default function HeroMagazine() {
  ...
  return (
    <section className="relative h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-black text-white">
      ...
      {/* 1. Imagen de Fondo y Capa de Oscuridad */}
      <div className="absolute inset-0 w-full h-full">
        {/* La Imagen: fill para ocupar todo, object-cover para no deformarse */}
        <Image 
          src="/hero-bg.jpg" // Ruta relativa a la carpeta 'public'
          alt="Lady with Dreams Background" 
          fill 
          className="object-cover"
          priority // Carga esta imagen primero
        />
        {/* La Capa (Overlay): bg-black con 75% de opacidad para dar protagonismo al texto */}
        <div className="absolute inset-0 bg-black/75 z-10"></div>
      </div>

      {/* 2. Contenedor Principal de Contenido */}
      {/* Añadimos z-20 para asegurar que el texto esté por encima de la capa negra */}
      <div className="relative z-20 flex flex-col items-center text-center px-4 w-full mt-10">
        ...
        >
          ...
          {/* "with Dreams" superpuesto */}
          <div className="absolute top-[60%] md:top-[65%] w-[120%] text-center">
            {/* Nota: italic da elegancia, pero para la tipografía manuscrita manuscrita
                clonada del diseño necesitaremos importar una fuente Script después. */}
            <span className="font-serif italic text-4xl md:text-7xl tracking-wide normal-case drop-shadow-2xl">
              with Dreams
            </span>
          </div>
        </motion.div>
        ...
        >
          <Link 
            href="#about" // Cambia esto a la sección de destino
